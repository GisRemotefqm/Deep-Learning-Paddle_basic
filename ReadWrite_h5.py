...
def resampling(dataset, outputpath, projection, flag=1, nodata=0, new_nodata=-1):
    ...
    gdal.Warp(outputpath, dataset, dstSRS=projection.ExportToWkt(),
...
if __name__ == '__main__':
    ...
    h5_path = r'G:\GF5_DPC_20200128_009164_L10000029922\GF5_DPC_20200128_009164_L10000029922_B865.h5'
    h5_filelist = get_h5filelist(h5_path)
    Idataset = get_h5Dataset(h5_filelist, 'I865P')
    imgx, imgy = get_RasterXY(Idataset)
    dataset_arr = get_RasterArr(Idataset, imgx, imgy)

    Lon = get_h5Dataset(h5_filelist, 'Longitude')
    Lat = get_h5Dataset(h5_filelist, 'Latitude')
    Lon_arr = get_RasterArr(Lon, imgx, imgy)[0]
    Lat_arr = get_RasterArr(Lat, imgx, imgy)[0]

    # define projection
    sinpro, singeo = defineSin()
    prosrs, geosrs = defineSRS(4326)

    # math projection
    transform = get_TransForm(Lon_arr, Lat_arr, sinpro, singeo)
    print(transform)

    # get sinusoidal tif
    writ_tif2(dataset_arr, sinpro, transform, 'D:\测试文件\sin_demo')

    # get WGS_84 tif
    # Idataset.SetProjection(sinpro.ExportToWkt())
    # Idataset.SetGeoTransform(transform)
    # Idata_list = get_RasterArr(Idataset, imgx, imgy)
    # resampling(Idataset, 'D:\测试文件\84_demo.tif', prosrs, 1, nodata=-32767)
